  ...
  return (
    <Link className='text-gray-700 cursor-pointer' to={id ? `/product/${id}` : '#'}>
      <div className='overflow-hidden'>
        <img className='hover:scale-110 transition ease-in-out w-full h-64 object-cover' src={productImage} alt={name || "Product"} />
      </div>
      <p className='pt-3 pb-1 text-sm'>{name || "No Name"}</p>
      <p className='text-sm font-medium'>{currency ? currency + price.toLocaleString() : "Price Not Available"}</p>
    </Link>
  );
};
