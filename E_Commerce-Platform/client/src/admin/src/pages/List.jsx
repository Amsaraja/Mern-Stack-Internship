        ...
        {/* Product List */}
        {
          list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm' key={index}>
              ...
              <button onClick={() => navigate(`/admin/edit/${item._id}`)} className='text-blue-600 hover:text-blue-800 cursor-pointer'>Edit</button>
              <p onClick={() => removeProduct(item._id)} className='text-red-600 hover:text-red-800 cursor-pointer text-center'>Delete</p>
            </div>
          ))
        }
      </div>
